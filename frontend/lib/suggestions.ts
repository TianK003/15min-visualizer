...
export async function fetchSuggestions(
  ...
  // Use caller-supplied bbox (e.g. gridDisk extent) or fall back to ~2 km default.
  const area: BBox = searchArea ?? {
    minLng: lng - 0.025,
    minLat: lat - 0.018,
    maxLng: lng + 0.025,
    maxLat: lat + 0.018,
  ...
  // Filter FRO polygons whose centroid falls within the search area,
  // sorted by distance to the demand centroid.
  const nearbyFro = froFeatures
    .map((f) => {
      ...
      if (clng < area.minLng || clng > area.maxLng || clat < area.minLat || clat > area.maxLat) return null;
